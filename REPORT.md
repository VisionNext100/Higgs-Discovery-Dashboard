...
- 标签分布：Signal 279,560（34.2%）/ Background 538,678（65.8%）。
- 数据划分（采用官方 KaggleSet）：
  | 集合     | 标记 | 数量    | 用途                 |
  | -------- | ---- | ------- | -------------------- |
...
| ------------------ | -------- | ------- | --------- | -------- |
| 多数类基准 (Dummy) | 0.658    | 0.500   | 1.079     | 绝对下限 |
| 逻辑回归           | 0.752    | 0.815   | 2.044     | 线性上限 |
...
| 多数类基准 | 0.658    | 0.000     | 0.000  | 0.000 | 0.5000  | 1.079     |
...
- **AMS 与 AUC 的权衡**：物理权重训练让 AMS 显著上升，但 AUC 反而略降。这说明 AUC（整体排序能力）与 AMS（信号富集区的发现显著性）并非同一目标——当训练聚焦于加权显著性时，模型在高分纯度区更“用力”，牺牲了部分全局排序。这进一步印证了 §6.1“评估指标必须与任务目标对齐”的观点。
- 若需同时兼顾排序与显著性，S7 是更均衡的折中方案。
...
### 13.1 课程知识覆盖
线性分类（逻辑回归）、PCA 降维、集成学习（随机森林 / XGBoost / Bagging / Stacking）、神经网络（MLP）、交叉验证与超参数搜索、模型评估、可解释机器学习（SHAP）、可信机器学习（鲁棒性）、数据可视化。
